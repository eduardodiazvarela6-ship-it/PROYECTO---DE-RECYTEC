...
function calcularTotales() {
  ...
  const ingresosP = state.ingresos.filter((i) => (i.periodo || p) === p);
  const gastosP = state.gastos.filter((g) => (g.periodo || p) === p);
  const totalIngresos = ingresosP.reduce((a, b) => a + Number(b.monto || 0), 0);
  const totalGastos = gastosP.reduce((a, b) => a + Number(b.monto || 0), 0);
  const hoy = todayISO();
  const ingresosHoy = state.ingresos.filter((i) => String(i.fecha || '').slice(0, 10) === hoy).reduce((a, b) => a + Number(b.monto || 0), 0);
  const gastosHoy = state.gastos.filter((g) => String(g.fecha || '').slice(0, 10) === hoy).reduce((a, b) => a + Number(b.monto || 0), 0);
  return { p, ingresosP, gastosP, totalIngresos, totalGastos, ganancia: totalIngresos - totalGastos, ingresosHoy, gastosHoy };
}
...
function renderGraficos() {
  ...
  state.charts.gastosDia = new Chart(el('chartGastosDia'), {
    type: 'line',
    data: { labels: labelsG, datasets: [{ label: 'Gastos', data: labelsG.map((l) => gastosDia[l]), borderColor: '#c23d3d', tension: .3 }] },
  ...
  state.charts.ingresosGastos = new Chart(el('chartIngresosGastos'), {
    ...
    data: {
      ...
      datasets: [{ data: [ingresosP.reduce((a, b) => a + Number(b.monto || 0), 0), gastosP.reduce((a, b) => a + Number(b.monto || 0), 0)], backgroundColor: ['#1f8f5f', '#c23d3d'] }]
    }
  });
  ...
  state.charts.periodos = new Chart(el('chartPeriodos'), {
    ...
    data: {
      ...
      datasets: [{ label: 'Ganancia', data: cerrados.map((p) => Number(p.ganancia || 0)), borderColor: '#156647', tension: .25 }]
    }
  });
}

function editableCell(value, field) {
  return `<input data-field="${field}" value="${String(value ?? '')}" />`;
}
...
function renderTablas() {
  ...
  el('tablaIngresos').innerHTML = t.ingresosP.map((r, i) => {
    const id = r.id || `ing-${i}`;
    return `<tr data-id="${id}"><td>${r.fecha || ''}</td><td>${editableCell(r.descripcion, 'descripcion')}</td><td>${editableCell(r.monto, 'monto')}</td><td>${editableCell(r.metodo_pago || 'efectivo', 'metodo_pago')}</td><td>${rowActions('INGRESOS', id)}</td></tr>`;
  }).join('');
  el('tablaGastos').innerHTML = t.gastosP.map((r, i) => {
    const id = r.id || `gas-${i}`;
    return `<tr data-id="${id}"><td>${r.fecha || ''}</td><td>${editableCell(r.descripcion, 'descripcion')}</td><td>${editableCell(r.monto, 'monto')}</td><td>${rowActions('GASTOS', id)}</td></tr>`;
  }).join('');

  el('tablaPeriodos').innerHTML = state.periodos.map((p, i) => {
    const id = p.id_periodo || `Periodo ${i + 1}`;
    return `<tr data-id="${id}"><td>${id}</td><td>${editableCell(p.fecha_inicio, 'fecha_inicio')}</td><td>${editableCell(p.fecha_fin, 'fecha_fin')}</td><td>${editableCell(p.total_ingresos || 0, 'total_ingresos')}</td><td>${editableCell(p.total_gastos || 0, 'total_gastos')}</td><td>${editableCell(p.ganancia || 0, 'ganancia')}</td><td>${editableCell(p.estado || 'cerrado', 'estado')}</td><td>${rowActions('PERIODOS', id)}</td></tr>`;
  }).join('');
}
...
function renderTrabajadores() {
  ...
  el('adelantosResumen').innerHTML = Object.entries(adelantosBy).map(([k, v]) => `<p><strong>${k}:</strong> ${money(v)}</p>`).join('') || '<p class="muted">Sin datos.</p>';

  const pendBy = groupTotals(state.prestamos, 'trabajador', (r) => String(r.estado).toLowerCase() !== 'pagado');
  el('prestamosResumen').innerHTML = Object.entries(pendBy).map(([k, v]) => `<p><strong>${k}:</strong> ${money(v)}</p>`).join('') || '<p class="muted">Sin datos.</p>';
  ...
  el('prestamosHistorial').innerHTML = state.prestamos.map((p, i) => {
    ...
    return `<tr><td>${p.fecha || ''}</td><td>${p.trabajador || ''}</td><td>${p.descripcion || ''}</td><td>${money(p.monto)}</td><td>${p.estado || 'pendiente'}</td>
<td>${pagado ? '<span class="muted">Pagado</span>' : `<button class="mini-btn btn-primary" onclick="marcarPrestamoPagado('${id}')">Marcar pagado</button>`}</td></tr>`;
  }).join('');
}

function llenarSelectPeriodo() {
  const select = el('exportPeriodo');
  select.innerHTML = `<option value="ALL">Todos</option>` + state.periodos.map((p) => `<option value="${p.id_periodo}">${p.id_periodo}</option>`).join('');
}
...
function collectRowData(tr) {
  const out = {};
  tr.querySelectorAll('input[data-field]').forEach((inp) => { out[inp.dataset.field] = inp.value; });
  return out;
}
...
function bindForms() {
  ...
  const bindSave = (formId, handler) => el(formId).addEventListener('submit', async (e) => {
    e.preventDefault();
  ...
  bindSave('gastoForm', async (data, btn) => apiPost({
    hoja: 'GASTOS', fecha: todayISO(), descripcion: data.descripcion, monto: Number(data.monto), periodo: state.currentPeriodo?.id_periodo || 'Periodo 1'
  }, btn));

  bindSave('ingresoForm', async (data, btn) => apiPost({
    hoja: 'INGRESOS', fecha: todayISO(), descripcion: data.descripcion, monto: Number(data.monto), metodo_pago: data.metodo_pago || 'efectivo', periodo: state.currentPeriodo?.id_periodo || 'Periodo 1'
  }, btn));

  bindSave('adelantoForm', async (data, btn) => apiPost({
    hoja: 'ADELANTOS', fecha: todayISO(), trabajador: data.trabajador, descripcion: data.descripcion, monto: Number(data.monto)
  }, btn));

  bindSave('prestamoForm', async (data, btn) => apiPost({
    hoja: 'PRESTAMOS', fecha: todayISO(), trabajador: data.trabajador, descripcion: data.descripcion, monto: Number(data.monto), estado: data.estado || 'pendiente'
  }, btn));

  document.querySelectorAll('.tab-btn').forEach((btn) => btn.addEventListener('click', () => abrirTab(btn.dataset.tab)));
  document.querySelectorAll('[data-go-tab]').forEach((btn) => btn.addEventListener('click', () => abrirTab(btn.dataset.goTab)));
